...
export class AuthController {
  signIn = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (
        !WalletHandler.isAddressOfPubkey(authData.address, authData.publicKey)
      ) {
        res.status(400).json({
          message: "Address does not match public key",
        });
        return;
      }
      ...
      const tokenExpiry = Math.floor(Date.now() / 1000) + 24 * 60 * 60; // 24 hours in seconds
      ...
      const tokens = authService.generateTokenPair({
        ...
        expiry: tokenExpiry, // Use our 24-hour expiry instead of the client's
      });
