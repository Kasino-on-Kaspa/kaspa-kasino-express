...
export class WithdrawalQueue {
	...
	private readonly MAX_RETRIES = 3; // Increased max retries
	...
	private async processNextTransaction(): Promise<void> {
		...
		try {
			...
			if (txHash) {
				...
				throw new Error(
					"Withdrawal function returned null unexpectedly"
				);
			}
		} catch (error: any) {
			...
			if (
				errorMessage.includes("was already accepted by the consensus")
			) {
			...
			if (nextItem.retryCount >= this.MAX_RETRIES) {
				...
				const isOrphanError = errorMessage.includes(
					"is an orphan where orphan is disallowed"
				);
				const isNoMatureUtxos = errorMessage.includes(
					"No mature UTXOs available for withdrawal"
				);
				...
				if (this.queue.length > 0) {
					...
					if (isNoMatureUtxos) {
						...
						console.log(
							"No mature UTXOs error - moved item to end of queue"
						);
					} else {
